...
-- ============================================================================
-- ALL MIGRATIONS INTEGRATED BELOW
-- ============================================================================
...
--              this point would silently wipe the real catalog every time â€”
...
--              persist â€” the previous login only checked fixed .env values).
...
-- Migration: 20260916_0021_fix_barcode_mismatch.sql
-- Description: Fixes an issue where a custom barcode sent from the frontend (e.g. CLAD...)
--              was silently ignored if an active barcode (e.g. PBV...) already existed in the registry.
...
-- ============================================================================
-- Migration: 20260920_0031_credit_sales.sql
-- Description: Adds "pay later" credit sales â€” a due date, outstanding/paid
--              status, and settlement timestamp on orders â€” so a credit sale
--              can be flagged, shown in an Outstanding Credits report, alarm
--              when its due date arrives, and be marked paid.
-- ============================================================================
...
-- Adds two trailing, defaulted params to complete_pos_sale_with_inventory so
-- existing callers are unaffected; only the POS checkout flow passes them
-- when the "Credit" payment method is selected.
CREATE OR REPLACE FUNCTION public.complete_pos_sale_with_inventory(
  p_customer_name TEXT,
  p_phone TEXT,
  p_address TEXT,
