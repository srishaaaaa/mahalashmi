        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-[#E5E7EB] bg-white p-3">
            ...
            <div className="grid grid-cols-7 gap-1">
              {days.map((day, idx) => (
                <div key={idx} className={`h-8 flex items-center justify-center rounded text-xs font-bold ${day === null ? '' : daysWithAnniversaries.has(day) ? 'relative border border-rose-500 bg-rose-50 text-[#111111]' : 'border border-[#E5E7EB] text-[#6B7280]'}`}>
                  {day}
                  {day && daysWithAnniversaries.has(day) && <div className="absolute bottom-0.5 h-1 w-1 rounded-full bg-rose-500"></div>}
                </div>
              ))}
            </div>
            <p className="mt-2 text-[10px] text-[#6B7280]">🔴 Dot = day with anniversary</p>
          </div>
