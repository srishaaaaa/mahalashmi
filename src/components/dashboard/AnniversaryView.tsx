...
export default function AnniversaryView() {
  ...
  const sendOffer = async (customer: CustomerRecord) => {
    const message = `Hi ${customer.name}! 💍 Happy Anniversary!\n\nHere's a special treat from us:\n20% OFF\n\nEnjoy your special day!`
    try {
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-[#E5E7EB] bg-white p-4">
            ...
            <div className="grid grid-cols-7 gap-2">
              {days.map((day, idx) => (
                <div key={idx} className={`aspect-square flex items-center justify-center rounded-lg text-sm font-bold ${day === null ? '' : daysWithAnniversaries.has(day) ? 'relative border-2 border-rose-500 bg-rose-50 text-[#111111]' : 'border border-[#E5E7EB] text-[#6B7280]'}`}>
                  {day}
                  {day && daysWithAnniversaries.has(day) && <div className="absolute bottom-1 h-1.5 w-1.5 rounded-full bg-rose-500"></div>}
                </div>
              ))}
            </div>
            <p className="mt-4 text-xs text-[#6B7280]">🔴 Dot = day with anniversary</p>
          </div>

          <div className="rounded-2xl border border-[#E5E7EB] bg-white p-4">
            <h3 className="mb-3 font-black text-[#273126]">Filter by upcoming anniversary</h3>
            ...
            <p className="mt-2 text-[10px] text-[#6B7280]">Shows customers whose next anniversary falls inside the chosen window.</p>
          </div>
        </div>
      )}
