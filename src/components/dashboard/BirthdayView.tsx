          ...
          <div className="space-y-4">
            <div className="rounded-2xl border border-[#E5E7EB] bg-white p-3">
              ...
              <div className="grid grid-cols-7 gap-0.5">
                {days.map((day, idx) => (
                  <div key={idx} className={`h-6 flex items-center justify-center rounded text-[10px] font-bold ${day === null ? '' : daysWithBirthdays.has(day) ? 'relative border border-pink-500 bg-pink-50 text-[#111111]' : 'border border-[#E5E7EB] text-[#6B7280]'}`}>
                    {day}
                    {day && daysWithBirthdays.has(day) && <div className="absolute bottom-0.5 h-0.5 w-0.5 rounded-full bg-pink-500"></div>}
                  </div>
                ))}
              </div>
              <p className="mt-2 text-[9px] text-[#6B7280]">🔴 Dot = birthday</p>
            </div>
      ...
      {currentPage === 'customers' && (
        <div className="space-y-4">
          <div className="rounded-2xl border border-[#E5E7EB] bg-white p-4">
            <h3 className="mb-3 font-bold text-[#273126] text-sm">Filter by upcoming birthday</h3>
