...
export default function Footer() {
  ...
  const shopPrimaryPhone = storeSettings?.phone || BRAND_PRIMARY_PHONE_DISPLAY
  ...
  const contactNumbers = [
    {
      ...
      display: shopPrimaryPhone,
      href: `tel:${shopPrimaryPhone.replace(/[^\d]/g, '')}`,
    },
    {
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 sm:py-14 grid grid-cols-1 md:grid-cols-4 gap-8 sm:gap-10">
        <div className="md:col-span-1">
          <div className="flex items-center gap-2 mb-4">
            <div className="w-9 h-9 rounded-full overflow-hidden shrink-0"><img src={logoUrl} alt={shopName} className="w-full h-full object-cover" /></div>
            <div>
              <p className="font-bold text-white text-base sm:text-lg font-headline leading-tight">{shopName}</p>
          ...
          <ul className="flex flex-col gap-4 text-sm">
            <li className="flex items-start gap-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                <span className="text-[11px] leading-relaxed text-gray-500 max-w-[260px]">{shopAddress}</span>
              </div>
            </li>
            ...
            <li className="flex items-start gap-3 flex-wrap">
              ...
              <a href={`mailto:${shopEmail}`} className="min-w-0 break-all hover:text-white transition-colors">{shopEmail}</a>
            </li>
          </ul>
        </div>
      </div>
